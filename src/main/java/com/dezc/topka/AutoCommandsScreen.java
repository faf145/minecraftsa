...
public class AutoCommandsScreen extends Screen {
    ...
    protected void init() {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            addDrawableChild(cb); checks.add(cb);

            TextFieldWidget tf = new TextFieldWidget(textRenderer, 30, y, width - 90, 20, Text.literal(""));
            tf.setText(cmd.command); tf.setMaxLength(256);
            addDrawableChild(tf); cmdFields.add(tf);

            TextFieldWidget df = new TextFieldWidget(textRenderer, width - 55, y, 45, 20, Text.literal(""));
            df.setText(String.valueOf(cmd.delay)); df.setMaxLength(3);
            addDrawableChild(df); delayFields.add(df);
        }
